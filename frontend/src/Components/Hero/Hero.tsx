...
const Hero: React.FC = () => {
  ...
  /* Inject keyframes and simulation */
  useEffect(() => {
    if (!styleInjected.current) {
      const el = document.createElement("style");
      el.textContent = KEYFRAMES;
  ...
  return (
    ...
    >
      ...
      {/* ── TICKER BAR (Sticky & Fully Transparent) ── */}
      <div
        style={{
          position: "sticky",
          top: 0,
      ...
      >
        ...
        <div style={{ animation: "slide-in-left 0.8s cubic-bezier(0.16,1,0.3,1) both" }}>
          ...
          <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: "clamp(42px, 5vw, 66px)", lineHeight: 1.08, color: textPrimary, marginBottom: 24 }}>
            Invest with <span style={{ color: "transparent", backgroundClip: "text", WebkitBackgroundClip: "text", backgroundImage: "linear-gradient(135deg, #10b981 0%, #0891b2 100%)" }}>clarity,</span><br />not noise.
          </h1>

          <p style={{ fontSize: 17, color: textSecondary, lineHeight: 1.75, maxWidth: 440, marginBottom: 40 }}>
            Track real-time stock data, read signal-filtered news, and discuss market moves — all without the noise.
          </p>

          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <Link to="/search" style={{ background: "linear-gradient(135deg, #10b981, #059669)", color: "white", padding: "14px 28px", borderRadius: 10, textDecoration: "none", fontWeight: 700, boxShadow: "0 4px 20px rgba(16,185,129,0.32)" }}>
              Start Investing Free
            </Link>
          </div>
        </div>
        ...
        {/* RIGHT COLUMN */}
        <div style={{ animation: "slide-in-right 0.8s cubic-bezier(0.16,1,0.3,1) both" }}>
          <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 20, padding: 28, position: "relative", animation: "card-float 5s ease-in-out infinite", boxShadow: cardShadow }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, background: greenBg, color: green, padding: "3px 10px", borderRadius: 4 }}>NVDA</span>
                <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 13, color: textSecondary, marginTop: 8 }}>NVIDIA Corporation</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 32, fontWeight: 500, color: textPrimary }}>${livePrice.toFixed(2)}</div>
              </div>
            </div>
            
            <svg viewBox="0 0 200 60" style={{ width: "100%", height: 60, marginBottom: 16 }}>
              <path d="M0,45 L20,40 L40,32 L60,35 L80,25 L100,18 L120,22 L140,14 L160,10 L180,8 L200,5" fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round" />
            </svg>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[{ l: "Market Cap", v: "$2.15T" }, { l: "P/E Ratio", v: "67.4" }, { l: "Vol (M)", v: "42.3M" }].map(({ l, v }) => (
                <div key={l} style={{ background: surface2, borderRadius: 10, padding: "10px 12px", border: `1px solid ${border2}` }}>
                  <div style={{ fontSize: 10, color: textMuted }}>{l.toUpperCase()}</div>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 14, color: textPrimary }}>{v}</div>
                </div>
              ))}
            </div>
      ...
      <div style={{ textAlign: "center", padding: "40px 0", position: "relative", zIndex: 10 }}>
        <span style={{ fontSize: 13, color: textMuted }}>Made with ❤️ by </span>
        <a href="https://github.com/nickonfix" target="_blank" style={{ color: green, fontWeight: 600, textDecoration: "none", fontFamily: "'DM Mono', monospace" }}>nickonfix</a>
      </div>
    </section>
  );
};
